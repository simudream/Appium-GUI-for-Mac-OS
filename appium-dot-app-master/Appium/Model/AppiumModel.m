...
-(BOOL)startServer
{
	...
	else
	{
		...
		command = [NSMutableString stringWithFormat:@"'/usr/local/bin/node'%@ lib/server/main.js", nodeDebugArguments];
	}
...
- (void)setupServerTask:(NSString *)commandString
{
	...
	{
		NSString * _currentDirectoryPath = [NSString stringWithFormat:@"%@/%@", @"/usr/local/lib", @"node_modules/appium"];
		NSLog(@"[Miao]: currentDirectoryPath > %@", _currentDirectoryPath);
		
		[self.serverTask setCurrentDirectoryPath:_currentDirectoryPath];
	}
