...
{
    return @"/usr/local/bin/node";//[NSString stringWithFormat:@"%@/%@", _nodeRootPath, @"node/bin/node"];
}
...
{
    return [NSString stringWithFormat:@"%@/%@/%@", @"/usr/local/lib", @"node_modules", packageName];
}
